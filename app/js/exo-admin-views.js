...
(function (A) {
  ...
  function formAkunBaru() {
    ...
    return '<div class="card elev-sm gap-10" id="akun-form"><div class="card-title">Akun admin baru</div><div class="grid g2" style="gap:8px">' +
      ...
      '<div class="field"><label>Unit kerja (menu yang tampil)</label><div class="flex wrap gap-6" style="padding-top:6px">' + A.UNIT_URUT.map(function (k) { var on = (f.unit || []).indexOf(k) >= 0; return '<button type="button" class="pill pill-sm' + (on ? ' on' : '') + '"' + aksi('akunUnit', k) + '>' + esc(A.UNIT_NAMA[k]) + '</button>'; }).join('') + '</div></div>' +
  ...
  VIEW.brand = function () {
    ...
    h += '<div><div class="t-115 up o-6" style="margin-bottom:9px">App mark</div><div class="flex gap-14 items-center"><div class="logo-box" style="width:88px;height:88px;flex:none"><img src="' + esc(b.markSrc) + '" alt=""></div><div class="grow stack gap-8"><label class="upload">Upload new mark<input type="file" accept="image/*" data-ubah="brandFile" data-arg="markSrc"></label><div class="t-115 o-6 lh-145"><b>Ukuran terbaik: 1024×1024 px</b> (min. 512×512), kotak 1:1, PNG/SVG latar transparan, maks. 1 MB. Set 48/96/192/512 dan ikon adaptif Android dibuat otomatis saat unggah.</div></div></div></div>';
    h += '<div><div class="t-115 up o-6" style="margin-bottom:9px">Wordmark &amp; tagline</div><div class="flex gap-14 items-center"><div class="logo-box grow" style="height:auto;min-height:62px;border-radius:20px;flex-direction:column;gap:4px;padding:10px 14px"><img src="' + esc(b.wordSrc) + '" alt="" style="height:34px">' + (b.tagline ? '<div style="font-size:9px;letter-spacing:.16em;text-transform:uppercase;color:var(--color-accent-700)">' + esc(b.tagline) + '</div>' : '') + '</div><label class="upload">Replace<input type="file" accept="image/*" data-ubah="brandFile" data-arg="wordSrc"></label></div>' +
      '<div class="t-115 o-6 lh-145" style="margin-top:8px"><b>Ukuran terbaik: 1200×300 px</b> (rasio 4:1, tinggi min. 160 px), PNG/SVG latar transparan, maks. 1 MB. Dipakai di header aplikasi, web depan, dan invoice.</div>' +
      '<div class="field" style="margin-top:10px"><label>Tagline (di bawah wordmark)</label><input class="input" value="' + esc(b.tagline == null ? '' : b.tagline) + '" maxlength="48" placeholder="We clean all purpose" data-ubah="brandTagline"><div class="t-11 o-6" style="margin-top:4px">Tampil huruf kapital kecil di bawah wordmark pada aplikasi pelanggan &amp; mitra, konsol admin, web depan, dan kartu referral. Kosongkan untuk menyembunyikan.</div></div></div>';
  ...
  AKSI.brandTagline = function (a, v) { S.brand.tagline = String(v || '').trim().slice(0, 48); S.brandDirty = true; };
