...
(function (X) {
  ...
  X.LAYAR.pbelajar = function () {
    var u = pesertaObj(), semua = L().untukPeserta(u), wajib = L().wajibBelum(u), sopBelum = L().sopBelum(u), sert = L().sertifikatPeserta(u.id), jalur = L().jalurUntuk(u);
    ...
    var h = '<div class="screen"><div class="hero hero--leaf"><div class="flex items-center gap-11">' + X.logoMark(36) + '<div class="grow"><div class="f-head t-17">Akademi EXOCLEAN</div><div class="t-115 o-7">Belajar bertahap, bersertifikat · ' + esc(L().namaFungsi(L().fungsiDari(u))) + '</div></div>' + X.tombolBahasa() + '</div>' +
      '<div class="flex gap-9" style="margin-top:16px"><div class="stat"><b>' + wajib.length + '</b><span>Wajib dasar</span></div><div class="stat"><b>' + sopBelum.length + '</b><span>SOP layanan</span></div><div class="stat"><b>' + sert.length + '</b><span>Sertifikat</span></div><div class="stat"><b>' + jam + ' jam</b><span>Selesai</span></div></div></div>';
    h += '<div class="stack gap-12" style="padding:18px 20px 0">';
    if (wajib.length) h += '<div class="card card-clay gap-4"><div class="f-head t-15">' + wajib.length + ' kursus wajib dasar belum tuntas</div><div class="t-115 lh-15 o-85">Kursus dasar (K3, etika, kebersihan tangan, ergonomi) menahan SEMUA job sampai lulus. Mulai dari: ' + esc(wajib[0].judul) + '.</div></div>';
    if (sopBelum.length) h += '<div class="card elev-sm gap-6"><div class="f-head t-15">SOP layanan · ' + sopBelum.length + ' belum lulus</div><div class="t-115 lh-15 o-7">Setiap SOP hanya menahan job layanan itu sendiri. Lulus SOP layanan yang paling sering Anda ambil dulu.</div><div class="flex gap-6 wrap">' + sopBelum.slice(0, 12).map(function (k) { return '<button class="pill pill-sm"' + aksi('lmsBuka', k.id) + '>' + esc(k.sop || k.kode) + ' · ' + esc((k.judul || '').replace(/^SOP [A-Z]-\d+ — /, '').slice(0, 22)) + '</button>'; }).join('') + (sopBelum.length > 12 ? '<span class="t-11 o-6">+' + (sopBelum.length - 12) + ' lagi di daftar kursus</span>' : '') + '</div></div>';
    if (jalur) {
  ...
  X.LAYAR.pmateri = function () {
    ...
    if (t.jenis === 'video' && t.kredit) h += '<div class="t-11 o-6">Sumber video: ' + esc(t.kredit) + ' · YouTube (disematkan, hak cipta milik pembuat)</div>';
