...
var EXO_TOKO = (function () {
  'use strict';
  var BIAYA_LAYANAN_BAWAAN = 0.05, TAHAN_OTOMATIS_HARI = 3;
  /* Biaya layanan penjual (%) — dari EXO_BIAYA (tingkat toko, kategori, program ongkir); cadangan 5% bila modul tidak dimuat. */
  function komisiPct(toko, kategori) { return window.EXO_BIAYA ? EXO_BIAYA.komisiPct(toko, kategori) : BIAYA_LAYANAN_BAWAAN * 100; }
  function komisiRp(toko, subtotal, kategori) { return window.EXO_BIAYA ? EXO_BIAYA.komisiRp(toko, subtotal, kategori) : Math.round(subtotal * BIAYA_LAYANAN_BAWAAN); }
  ...
  var ATURAN_BAWAAN = { gratisOngkirMin:150000, gratisOngkirMaks:20000, gratisOngkirHanyaPeserta:false, asuransiAktif:true, asuransiPct:0.5, asuransiMin:2500, asuransiBulat:500, asuransiBawaan:true, asuransiBolehDilepas:true, asuransiWajibKategori:['mesin'], asuransiWajibMinHarga:2000000, proteksiAktif:true, proteksiPct:5, proteksiMinHarga:500000, proteksiKategori:['mesin'], proteksiBulan:12, biayaJasa:1000, biayaJasaGratisMetode:['wallet'], bebasTransaksiPertama:4, biayaLayananVA:1000, poinMaksPct:10 };
  /* Aturan berlaku dibaca dari EXO_BIAYA (disetel admin → Biaya & asuransi) setiap kali dipakai, sehingga perubahan langsung terasa di checkout. */
  function AT() { return window.EXO_BIAYA ? EXO_BIAYA.aturanToko() : ATURAN_BAWAAN; }
  ...
  function katalogFilter(q, kategori, f, titik) {
    ...
    var daftar = semua.filter(function (p) {
      if (f.gratisOngkir && p.hargaJual < AT().gratisOngkirMin) return false;
  ...
  function opsiKurir(t, tarif, ekstra) {
    ...
    if (ekstra && ekstra.mitra) daftar.unshift({ id:'mitra', nama:'Dibawa mitra saat kunjungan' + (ekstra.mitraNama ? ' · ' + ekstra.mitraNama : ''), harga:0, statis:true, mitra:true });
  ...
  function proteksiBoleh(p, harga) { var a = AT(); return a.proteksiAktif && (a.proteksiKategori.indexOf(p.kategori) >= 0 || harga >= a.proteksiMinHarga); }
  /* Jumlah transaksi (batch) pembeli yang pernah dibuat — untuk bebas biaya jasa N transaksi pertama (pola pembeli baru Tokopedia). */
  function jumlahTransaksi(pembeliId) { var d = db(); if (!d || !pembeliId) return 0; var b = {}; d.all('pesananToko').forEach(function (o) { if (o.pembeliId === pembeliId && o.status !== 'menunggu-bayar' && o.status !== 'dibatalkan') b[o.batch || o.id] = 1; }); return Object.keys(b).length; }
  ...
  function hitungKeranjang(items, kuponKode, kurirId, tarif, opsi) {
    ...
    (items || []).forEach(function (it) { var p = produk(it.produkId); if (!p) return; var v = (p.varian || []).filter(function (x) { return x.nama === it.varian; })[0] || p.varian[0] || { nama:'', harga:p.harga, stok:p.stok }; var g = perToko[p.tokoId] = perToko[p.tokoId] || { toko:p.toko, items:[], subtotal:0, ongkir:0, diskon:0, total:0, kupon:null, galat:[], peringatan:'', asuransi:0, proteksi:0, diskonOngkir:0, gratisOngkir:0, kuponOngkir:0, catatan:(opsi.catatan || {})[p.tokoId] || '' }; var harga = hargaSetelahDiskon(p, v), sub = harga * it.qty, kunci = p.id + '|' + v.nama, boleh = proteksiBoleh(p, harga), pHarga = boleh ? bulat(harga * AT().proteksiPct / 100, 100) : 0, pAktif = boleh && !!(opsi.proteksi || {})[kunci]; if (it.qty > v.stok) g.galat.push(p.nama + ' (' + v.nama + ') stok hanya ' + v.stok); g.items.push({ produkId:p.id, nama:p.nama, varian:v.nama, qty:it.qty, harga:harga, hargaAsli:v.harga, diskonPct:p.diskonPct || 0, ikon:p.ikon, berat:p.berat || 500, kategori:p.kategori, asuransiWajib:!!p.asuransiWajib, proteksiBoleh:boleh, proteksiHarga:pHarga, proteksi:pAktif, kunci:kunci }); g.subtotal += sub; if (pAktif) g.proteksi += pHarga * it.qty; });
    Object.keys(perToko).forEach(function (tid) {
      var g = perToko[tid], opsiK = opsiKurir(g.toko, tarif, (opsi.kurirMitra || {})[tid] ? { mitra:true, mitraNama:opsi.mitraNama } : null), pilih = typeof kurirId === 'object' && kurirId ? kurirId[tid] : kurirId;
      g.alamat = (opsi.alamatPerToko || {})[tid] || null;
      ...
      var at = AT(), dikirim = kr.id !== 'ambil' && kr.id !== 'mitra', pilihan = (opsi.asuransi || {})[tid];
      g.asuransiTersedia = at.asuransiAktif && dikirim; g.asuransiWajib = g.asuransiTersedia && (window.EXO_BIAYA ? EXO_BIAYA.asuransiWajib(g.items, g.subtotal) : false);
      g.asuransiAktif = g.asuransiTersedia && (g.asuransiWajib || !at.asuransiBolehDilepas || (pilihan == null ? at.asuransiBawaan : pilihan !== false));
      g.asuransiHarga = window.EXO_BIAYA ? EXO_BIAYA.premiAsuransi(g.subtotal) : Math.max(at.asuransiMin, bulat(g.subtotal * at.asuransiPct / 100, at.asuransiBulat)); g.asuransi = g.asuransiAktif ? g.asuransiHarga : 0;
      ...
      g.gratisOngkir = window.EXO_BIAYA ? EXO_BIAYA.subsidiOngkir(g.subtotal, g.ongkir, g.toko) : (g.ongkir > 0 && g.subtotal >= at.gratisOngkirMin ? Math.min(g.ongkir, at.gratisOngkirMaks) : 0);
      g.komisiPct = komisiPct(g.toko); g.biayaLayanan = komisiRp(g.toko, g.subtotal);
    ...
    var metode = opsi.metode || 'wallet', A0 = AT(), nTx = opsi.nTransaksi != null ? Number(opsi.nTransaksi) : jumlahTransaksi(opsi.pembeliId), biayaJasa = window.EXO_BIAYA ? EXO_BIAYA.biayaJasaPembeli(metode, nTx) : (metode === 'wallet' ? 0 : A0.biayaJasa), biayaLayananVA = window.EXO_BIAYA ? EXO_BIAYA.biayaLayananVA(metode, opsi.hariDaftar) : 0, subTotal = daftar.reduce(function (n, g) { return n + g.total; }, 0);
    var poinMaks = Math.min(Number(opsi.poinTersedia) || 0, Math.floor(subTotal * A0.poinMaksPct / 100)), poinDipakai = opsi.poin ? (opsi.poin === true ? poinMaks : Math.min(poinMaks, Number(opsi.poin) || 0)) : 0;
    var total = Math.max(0, subTotal + biayaJasa + biayaLayananVA - poinDipakai);
    var ringkas = { barang:daftar.reduce(function (n, g) { return n + g.items.reduce(function (m, it) { return m + it.qty; }, 0); }, 0), hargaBarang:daftar.reduce(function (n, g) { return n + g.subtotal; }, 0), ongkir:daftar.reduce(function (n, g) { return n + g.ongkir; }, 0), asuransi:daftar.reduce(function (n, g) { return n + g.asuransi; }, 0), proteksi:daftar.reduce(function (n, g) { return n + g.proteksi; }, 0), biayaJasaAsli:A0.biayaJasa, biayaJasa:biayaJasa, biayaJasaAlasan:biayaJasa ? '' : (A0.biayaJasaGratisMetode.indexOf(metode) >= 0 ? 'gratis untuk ' + metode : nTx < A0.bebasTransaksiPertama ? 'gratis ' + A0.bebasTransaksiPertama + ' transaksi pertama' : ''), biayaLayananVA:biayaLayananVA, diskonOngkir:daftar.reduce(function (n, g) { return n + g.diskonOngkir; }, 0), diskonKupon:daftar.reduce(function (n, g) { return n + g.diskon; }, 0), diskonProduk:daftar.reduce(function (n, g) { return n + g.diskonProduk; }, 0), poinDipakai:poinDipakai, poinMaks:poinMaks };
    ringkas.promo = ringkas.diskonOngkir + ringkas.diskonKupon + ringkas.poinDipakai; ringkas.hemat = ringkas.promo + ringkas.diskonProduk + (A0.biayaJasa - biayaJasa);
  ...
  function checkout(items, kuponKode, kurirId, pembeli, alamat, catatan, tarif, opsi) {
    ...
    var dibuat = [], batch = 'B' + Date.now().toString(36), metode = h.metode, sisaPoin = h.ringkas.poinDipakai, sisaJasa = h.ringkas.biayaJasa, sisaVA = h.ringkas.biayaLayananVA || 0;
    h.toko.forEach(function (g) {
      var n = d.nextNo('pesananToko'), o = d.insert('pesananToko', { no:'TK-' + (1000 + n), tokoId:g.toko.id, pembeliId:pembeli.id || null, pembeliNama:pembeli.nama, items:g.items, subtotal:g.subtotal, ongkir:g.ongkir, diskon:g.diskon, kupon:g.kupon ? g.kupon.kode : '', total:g.total, biayaLayanan:g.biayaLayanan != null ? g.biayaLayanan : komisiRp(g.toko, g.subtotal), komisiPct:g.komisiPct != null ? g.komisiPct : komisiPct(g.toko), kurir:g.kurir.nama, kurirId:g.kurir.id, kurirKode:g.kurir.kurir || '', layanan:g.kurir.layanan || '', kurirEtd:g.kurir.etd || '', berat:g.berat, resi:'', status:metode === 'wallet' ? 'baru' : 'menunggu-bayar', alamat:(g.alamat && g.alamat.full) || alamat, titik:(g.alamat && g.alamat.point) || pembeli.titik || null, serahMitra:g.kurir.id === 'mitra' ? { nama:opsi.mitraNama || 'mitra EXOCLEAN', jadwal:opsi.mitraJadwal || '' } : null, pembeliTelp:pembeli.telp || '', catatan:g.catatan || catatan || '', bayar:metode, batch:batch, asuransi:g.asuransi, proteksi:g.proteksi, diskonOngkir:g.diskonOngkir, gratisOngkir:g.gratisOngkir, poinDipakai:Math.min(sisaPoin, g.total), biayaJasa:sisaJasa, biayaLayananVA:sisaVA, asuransiWajib:!!g.asuransiWajib, at:kini(), contoh:false });
      if (window.EXO_INVOICE) { try { var inv = EXO_INVOICE.terbitkan('toko', o.id, EXO_INVOICE.dariPesananToko(o, g.toko)); if (inv) d.update('pesananToko', o.id, { invoiceNo:inv.no }); } catch (e) { /* struk opsional */ } }
      sisaPoin = Math.max(0, sisaPoin - o.poinDipakai); sisaJasa = 0; sisaVA = 0;
  ...
  /* Simpan produk (pola Tokopedia Seller): foto ≤ 5, nama ≤ 70, deskripsi ≤ 2000,
     varian ≤ 2 tipe × 10 opsi dengan harga/stok/SKU per kombinasi, grosir,
     minimum beli, pre-order 1–90 hari, berat & dimensi, asuransi wajib, kurir.
     Produk baru → moderasi; isi.draf → draf; produk aktif yang disunting tetap aktif. */
  /* Simpan produk (pola Tokopedia Seller): foto ≤ 5, nama ≤ 70, deskripsi ≤ 2000,
     varian ≤ 2 tipe × 10 opsi dengan harga/stok/SKU per kombinasi, grosir,
     minimum beli, pre-order 1–90 hari, berat & dimensi, asuransi wajib, kurir.
     Produk baru → moderasi; isi.draf → draf; produk aktif yang disunting tetap aktif. */
  function simpanProduk(tokoId, isi) {
    var d = db(), lama = isi.id ? d.find('produk', isi.id) : null;
    var bersih = { tokoId:tokoId, nama:String(isi.nama || '').trim().slice(0, 70), kategori:isi.kategori || 'alat', deskripsi:String(isi.deskripsi || '').trim().slice(0, 2000), etalase:String(isi.etalase || 'Umum').trim() || 'Umum', merek:String(isi.merek || '').trim().slice(0, 40), kondisi:isi.kondisi === 'bekas' ? 'bekas' : 'baru', unggulan:!!isi.unggulan, ikon:isi.ikon || '📦', foto:(isi.foto || []).filter(Boolean).slice(0, 5), video:/^https:\/\/(www\.)?(youtube\.com|youtu\.be|youtube-nocookie\.com)\//.test(String(isi.video || '')) ? String(isi.video).trim() : '', berat:Math.max(1, Math.min(50000, Number(isi.berat) || 500)), dimensi:{ p:Number((isi.dimensi || {}).p) || 0, l:Number((isi.dimensi || {}).l) || 0, t:Number((isi.dimensi || {}).t) || 0 }, diskonPct:Math.max(0, Math.min(90, Number(isi.diskonPct) || 0)), minBeli:Math.max(1, Number(isi.minBeli) || 1), preorder:isi.preorder && isi.preorder.aktif ? { aktif:true, hari:Math.min(90, Math.max(1, Number(isi.preorder.hari) || 7)) } : false, asuransiWajib:!!isi.asuransiWajib, kurir:Array.isArray(isi.kurir) && isi.kurir.length ? isi.kurir.slice(0, 5) : null, grosir:(isi.grosir || []).filter(function (g) { return Number(g.min) > 1 && Number(g.harga) > 0; }).slice(0, 3).map(function (g) { return { min:Number(g.min), harga:Number(g.harga) }; }), varianTipe:(isi.varianTipe || []).slice(0, 2).map(function (t) { return { nama:String(t.nama || '').trim().slice(0, 20), opsi:(t.opsi || []).map(function (o) { return String(o).trim().slice(0, 30); }).filter(Boolean).slice(0, 10) }; }).filter(function (t) { return t.nama && t.opsi.length; }),
      varian:(isi.varian || []).map(function (v) { return { nama:String(v.nama || 'Standar').trim(), harga:Number(v.harga) || 0, stok:Math.max(0, Number(v.stok) || 0), sku:String(v.sku || '').trim().slice(0, 40) }; }).filter(function (v) { return v.nama && v.harga > 0; }) };
    if (!bersih.nama || bersih.nama.length < 4) throw new Error('Nama produk minimal 4 karakter');
    if (!isi.draf && !bersih.varian.length) throw new Error('Minimal satu varian dengan harga > 0');
    if (bersih.varian.length) { bersih.harga = Math.min.apply(null, bersih.varian.map(function (v) { return v.harga; })); bersih.stok = bersih.varian.reduce(function (s, v) { return s + v.stok; }, 0); } else { bersih.harga = Number(isi.harga) || 0; bersih.stok = 0; }
    /* volumetrik: berat tagih = maks(berat, P×L×T/6000 kg) */
    var vol = bersih.dimensi.p * bersih.dimensi.l * bersih.dimensi.t / 6; bersih.beratTagih = Math.max(bersih.berat, Math.round(vol));
    if (lama) { var status = isi.draf ? 'draf' : (lama.status === 'aktif' && isi.tetapAktif ? 'aktif' : 'moderasi'); return d.update('produk', isi.id, Object.assign(bersih, { status:status, rev:(lama.rev || 1) + 1 })); }
    return d.insert('produk', Object.assign({ status:isi.draf ? 'draf' : 'moderasi', terjual:0, rating:0, rev:1 }, bersih));
  }
  ...
  function keuanganToko(tokoId) {
    ...
    var bruto = selesai.reduce(function (n, o) { return n + o.subtotal - (o.refund || 0); }, 0), biaya = selesai.reduce(function (n, o) { return n + (o.biayaLayanan || Math.round(o.subtotal * BIAYA_LAYANAN_BAWAAN)); }, 0), ongkir = selesai.reduce(function (n, o) { return n + o.ongkir; }, 0);
    ...
    /* Isi saldo iklan yang dipotong dari saldo toko (EXO_IKLAN) ikut mengurangi saldo tersedia. */
    var iklan = d ? d.all('iklanTopup').filter(function (p) { return p.tokoId === tokoId && p.sumber === 'saldo' && p.status === 'dibayar'; }).reduce(function (n, p) { return n + p.jumlah; }, 0) : 0;
    return { bruto:bruto, biayaLayanan:biaya, ongkir:ongkir, bersih:bruto - biaya + ongkir, tertahan:tertahan.reduce(function (n, o) { return n + o.total; }, 0), nTertahan:tertahan.length, ditarik:ditarik, iklan:iklan, saldo:bruto - biaya + ongkir - ditarik - iklan, pesananSelesai:selesai.length, penarikan:d ? d.all('penarikanToko').filter(function (p) { return p.tokoId === tokoId; }).slice().reverse() : [] };
  }
  ...
  function ringkasanAdmin() {
    ...
    var selesai = ps.filter(function (o) { return o.status === 'selesai'; }), gmv = selesai.reduce(function (n, o) { return n + o.subtotal; }, 0), komisi = selesai.reduce(function (n, o) { return n + (o.biayaLayanan || Math.round(o.subtotal * BIAYA_LAYANAN_BAWAAN)); }, 0);
