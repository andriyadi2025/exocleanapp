...
(function (X) {
  ...
  X.LAYAR.home = function () {
    ...
    if (window.EXO_PERJALANAN) h += '<div style="padding:18px 20px 0"><button class="card elev-sm gap-3" style="text-align:start;cursor:pointer;width:100%"' + aksi('ke', 'perjalanan') + '><div class="flex items-center gap-8"><span class="tag tag-accent">Perjalanan</span><span class="t-11 o-6">✈️ 🏨 🚆 🚌 🚢 🚗 🗺️ 🕌 📦</span><span class="t-11 o-6" style="margin-inline-start:auto">→</span></div><div class="t-125">Tiket pesawat, hotel, kereta, bus, kapal, sewa mobil, tur, umroh & kargo · Darmawisata</div></button></div>';
    if (window.EXO_PPOB) h += '<div style="padding:18px 20px 0"><button class="card elev-sm gap-3" style="text-align:start;cursor:pointer;width:100%"' + aksi('ke', 'tagihan') + '><div class="flex items-center gap-8"><span class="tag tag-accent-2">Bayar & Isi Ulang</span><span class="t-11 o-6">PLN · BPJS · PDAM · pulsa · data · token</span><span class="t-11 o-6" style="margin-inline-start:auto">→</span></div><div class="t-125">Bayar tagihan dan isi ulang dari EXO Wallet · Darmawisata H2H</div></button></div>';
  ...
  X.LAYAR.success = function () {
    ...
    h += '<div class="card elev-sm gap-9" style="margin-top:22px;width:100%;text-align:start">' +
      '<div class="kv"><span>' + esc(tx('Order')) + '</span><span class="f-head">' + esc(K.orderNo) + '</span></div>' + (function () { if (!window.EXO_INVOICE) return ''; var inv = EXO_INVOICE.untukRef('jasa', K.orderNo); if (!inv) { try { var al = X.alamatKini(), vo = X.voucherApplied() ? X.voucherKini().amount : 0; inv = EXO_INVOICE.terbitkan('jasa', K.orderNo, EXO_INVOICE.dariJasa({ no:K.orderNo, pembeli:'Dewi Anggraini', telp:K.telp || '081234567002', alamat:al ? al.full : '', jadwal:X.ringkasSlot(), mitra:j.name, layanan:I.svcName(K.jasa) + ' · ' + X.qtyText(K.jam), qty:1, harga:X.subtotalN() - X.addonTotal(), tambahan:[], subtotal:X.subtotalN(), voucher:vo, diskonLangganan:X.diskonLangganan(), biayaAplikasi:D.PLATFORM_FEE, total:X.totalN(), metode:b.name })); } catch (e) { inv = null; } } return inv ? '<div class="kv"><span>Invoice</span><a class="t-11" style="font-variant-numeric:tabular-nums" href="' + EXO_INVOICE.tautan(inv.no) + '" target="_blank" rel="noopener">' + esc(inv.no) + '</a></div>' : ''; })() +
