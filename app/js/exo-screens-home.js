...
(function (X) {
  ...
  /* Korsel banner generik (dipakai beranda, halaman Toko, beranda mitra): daftar dari konten. */
  X.korselBanner = function (daftar, gaya) { var W = window.EXO_KONTEN ? EXO_KONTEN.WARNA : {}; var d = (daftar || []).filter(function (b) { return b.aktif !== false; }); if (!d.length) return ''; return '<div class="hscroll" style="gap:10px' + (gaya ? ';' + gaya : '') + '">' + d.map(function (bn) { return '<button style="all:unset;box-sizing:border-box;cursor:pointer;flex:none;width:262px;height:112px;border-radius:18px;padding:14px 16px;color:#fff;background:' + (W[bn.warna] || bn.warna || W.hijau) + ';display:flex;flex-direction:column;justify-content:space-between;position:relative;overflow:hidden"' + aksi('ke', bn.ke) + '><span style="position:absolute;right:-6px;top:-8px;font-size:64px;opacity:.25">' + esc(bn.ikon || '') + '</span><div><div style="font-weight:800;font-size:16px;line-height:1.15">' + esc(bn.judul) + '</div><div style="font-size:11.5px;opacity:.9;margin-top:3px">' + esc(bn.sub) + '</div></div><span style="align-self:flex-start;background:#fff;color:#0b5f52;font-size:11px;font-weight:700;padding:5px 10px;border-radius:999px">' + esc(bn.cta) + ' →</span></button>'; }).join('') + '</div>'; };
  X.LAYAR.home = function () {
    var b = window.EXO_BRAND ? EXO_BRAND.baca() : {}, j = X.juruKini(), cari = (K.cari || '').trim().toLowerCase(), alamat = X.alamatKini();
    var KT = window.EXO_KONTEN ? EXO_KONTEN.baca('klien') : null, banner = KT ? KT.banner.filter(function (x) { return x.aktif !== false; }).map(function (x) { return { warna:(EXO_KONTEN.WARNA[x.warna] || x.warna || EXO_KONTEN.WARNA.hijau), judul:x.judul, sub:x.sub, cta:x.cta, ke:x.ke, ikon:x.ikon }; }) : BANNER, menu = KT ? KT.menu.filter(function (m) { return m.aktif !== false; }).map(function (m) { return { id:m.id, jasa:!!m.jasa, ke:m.ke, ikon:m.ikon, label:m.label }; }) : MENU;
    var h = '<div class="screen">';
    /* bilah atas: cari + keranjang + lonceng */
    h += '<div style="padding:14px 16px 0;background:linear-gradient(180deg,var(--color-accent-2-100,#e6f4ef),var(--color-bg) 90%)"><div class="flex items-center gap-8"><div class="searchbox" style="background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.06)">' + ikon(IKON.cari, 15) + '<input id="exo-cari" data-simpan="cari" data-gambar="1" value="' + esc(K.cari || '') + '" placeholder="' + esc(KT && KT.cariPlaceholder || 'Cari jasa, produk, tagihan…') + '" aria-label="Cari">' + (cari ? '<button class="hapus"' + aksi('cariKosong') + ' aria-label="Hapus">✕</button>' : '') + '</div>' +
      ...
      '<button class="btn btn-icon btn-secondary btn-plain"' + aksi('ke', 'notifikasi') + ' aria-label="Notifikasi">' + ikon(IKON.lonceng) + '</button><button class="lang-btn"' + aksi('ke', 'lang') + '>' + (K.lang || 'id').toUpperCase() + '</button></div>';
    ...
    if (window.EXO_KONTEN) h += EXO_KONTEN.tiketHtml('klien', 'margin-top:10px'); else if (b.tickerOn && b.tickerText) h += '<div class="ticker" style="margin-top:10px"><div class="ticker-badge"><i></i>' + esc(b.tickerBadge) + '</div><div class="ticker-win"><div class="ticker-track"><span>' + esc(b.tickerText) + '</span><span>' + esc(b.tickerText) + '</span></div></div></div>';
    h += '</div>';
    ...
    if (cari) {
      ...
      h += '<div class="t-115 up o-6">Jasa · ' + jasaCocok.length + '</div>' + (jasaCocok.length ? '<div class="svc-grid">' + jasaCocok.map(function (s) { var ov = window.EXO_KONTEN ? EXO_KONTEN.ikonLayanan('klien', s.id) : null; return '<button class="svc' + (s.daun ? ' leaf' : '') + '"' + aksi('pilihJasa', s.id) + '><i>' + (ov ? EXO_KONTEN.ikonHtml(ov, 22) : ikon(s.d, 20)) + '</i><b>' + esc(I.svcName(s.id)) + '</b></button>'; }).join('') + '</div>' : '<div class="t-125 o-6">Tidak ada jasa yang cocok.</div>');
    ...
    bag.toko = '<div style="padding:18px 0 0 16px"><div class="flex items-center gap-8" style="padding-right:16px"><div class="f-head t-17">' + esc(KT && KT.judulToko || 'Toko pilihan') + '</div><button class="btn btn-ghost t-115" style="margin-inline-start:auto;color:#0a8f5c"' + aksi('ke', 'toko') + '>Lihat semua →</button></div><div class="hscroll" style="gap:8px;margin-top:8px;padding-right:16px">' + toko.map(function (x) { var sk = T().skorToko(x.id); return '<button class="card elev-sm gap-3" style="width:160px;text-align:start;cursor:pointer"' + aksi('tokoLihat', x.id) + '><div class="flex items-center gap-6">' + (x.badge === 'official' ? '<span class="tag tag-accent" style="font-size:10px">Official</span>' : x.badge === 'power' ? '<span class="tag tag-accent-2" style="font-size:10px">Power</span>' : '<span class="tag tag-neutral" style="font-size:10px">Toko</span>') + '</div><div class="t-125 bold lh-14" style="min-height:32px">' + esc(x.nama) + '</div><div class="t-11 o-6">⭐ ' + sk.rating + ' · ' + esc(x.kota || '') + '</div></button>'; }).join('') + '</div></div>';
    var dekat = X.daftarJuru().slice(0, 6);
    bag.mitra = '<div style="padding:18px 0 0 16px"><div class="flex items-center gap-8" style="padding-right:16px"><div class="f-head t-17">' + esc(t('nearYou')) + '</div><button class="btn btn-ghost t-115" style="margin-inline-start:auto;color:#0a8f5c"' + aksi('pilihJasa', 'hourly') + '>' + esc(t('seeAll')) + ' →</button></div><div class="hscroll" style="gap:10px;margin-top:8px;padding-right:16px">' + dekat.map(function (cl) { return '<div class="near"><div class="flex items-center gap-8">' + X.avJuru(cl, 34, 'leaf') + '<div class="t-12 bold" style="line-height:1.2">' + esc(X.namaDepan(cl)) + '</div></div><div class="t-115 o-7" style="margin-top:9px">' + (cl.rating ? '★ ' + esc(cl.rating) + ' · ' : '') + esc(cl.jobs) + ' ' + esc(tx('jobs')) + '</div><div class="f-head t-14" style="margin-top:5px">' + rp(X.rateFor(cl)) + '</div><div class="t-11 o-6">' + esc(cl.distance ? cl.distance + ' ' + tx('away') : (cl.years ? cl.years + ' ' + tx('with EXOCLEAN') : tx('new'))) + '</div></div>'; }).join('') + '</div></div>';
    bag.jaminan = '<div style="padding:16px 16px 0"><div class="card card-leaf gap-6" style="flex-direction:row;align-items:center"><span class="av av-leaf" style="--s:30px">' + ikon(IKON.perisaiCentang, 15) + '</span><div class="t-115 lh-145 o-85"><b>' + esc(t('guarantee')) + '.</b> ' + esc(KT && KT.jaminan || tx('Once confirmed, only you can move the time. If we ever reschedule you, Rp100.000 credit lands in your wallet the same minute — no ticket, no chasing.')) + '</div></div></div>';
    var rek = T().rekomendasi(K.tokoDilihat || [], K.keranjang || [], 8);
    bag.rekomendasi = '<div style="padding:18px 16px 0"><div class="f-head t-17" style="color:#0a8f5c">' + esc(KT && KT.judulRekomendasi || 'Rekomendasi untuk kamu') + '</div><div style="width:72px;height:2px;background:#0a8f5c;margin:4px 0 10px"></div><div class="grid-2">' + (X.kartuIklanDaftar ? X.kartuIklanDaftar('beranda', {}) : '') + rek.map(kartu).join('') + '</div><button class="btn btn-secondary btn-block" style="margin:12px 0 0"' + aksi('ke', 'toko') + '>Lihat semua produk</button></div>';
    var urutan = (KT && KT.urutan && KT.urutan.length ? KT.urutan : ['kunjungan', 'flash', 'toko', 'mitra', 'jaminan', 'rekomendasi']).slice();
    if (urutan.indexOf('flashJasa') < 0) { var posF = urutan.indexOf('flash'); urutan.splice(posF < 0 ? 1 : posF, 0, 'flashJasa'); }   /* Flash Deal jasa selalu di atas Flash Sale produk */
    urutan.forEach(function (nama) { if (!KT || KT.bagian[nama] !== false) h += bag[nama] || ''; });
    Object.keys(bag).forEach(function (nama) { if (urutan.indexOf(nama) < 0 && (!KT || KT.bagian[nama] !== false)) h += bag[nama]; });
    h += '<div class="spacer-26"></div>';
    return h + '</div>';
  };
