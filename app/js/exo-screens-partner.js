...
(function (X) {
  ...
  X.profilMitra = function () {
    ...
    h += '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px 6px">' + [['pbelajar', '🎓', 'Akademi'], ['pinsentif', '🎯', 'Insentif'], ['pabsen', '🕒', 'Absen'], ['priwayat', '🧾', 'Riwayat'], ['pkontak', '🆘', 'Kontak darurat'], ['palat', '🧰', 'Alat kerja'], ['ptas', '🎒', 'Tas & isi ulang'], ['cs', '🤖', 'Bantuan AI'], ['privasi', '🔏', 'Privasi & data'], ['pinKelola', '🔢', 'PIN transaksi'], ['duaFaktor', '🔐', 'Dua langkah'], ['perangkat', '📲', 'Perangkat']].map(function (m) { return '<button style="all:unset;box-sizing:border-box;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:6px;text-align:center"' + aksi('ke', m[0]) + '><span style="width:50px;height:50px;border-radius:16px;background:var(--color-surface);display:grid;place-items:center;font-size:23px;box-shadow:0 1px 2px rgba(0,0,0,.05)">' + m[1] + '</span><span class="t-11">' + m[2] + '</span></button>'; }).join('') + '</div>';
  ...
  if (window.EXO_KESELAMATAN) { var kinVerifAsli = null; setTimeout(function () { kinVerifAsli = X.AKSI.kinVerif; if (!kinVerifAsli) return; X.AKSI.kinVerif = function (i) { var r = kinVerifAsli.apply(this, arguments); try { EXO_KESELAMATAN.simpanKontak(aku().name, K.kin); } catch (e) { /* abaikan */ } return r; }; }, 0); }
  X.kartuPengumuman = kartuPengumuman; X.kartuAkademi = kartuAkademi; X.gerbangSop = gerbangSop;
})(ExoApp);
