...
(function (X) {
  ...
  A.pinResetOtpPeriksa = function () { var f = K.pinKelola; EXO_SERVER.otpPeriksa(f.telp, f.kode, sisi() === 'partner' ? 'mitra' : sisi() === 'toko' ? 'toko' : 'klien').then(function (r) { if (!r.ok) { f.pesan = r.error || 'Kode salah.'; X.gambar(); return; } if (r.data && r.data.sesi && window.EXO_BRANKAS) EXO_BRANKAS.terimaSesi(r.data.sesi); f.tahap = 'telp'; f.kode = ''; K.pinGerbang = { alasan:'', kerja:K.pinResetTugas || null, tahap:'reset', isi:'', isi2:'', pesan:'', sibuk:false }; K.pinResetTugas = null; K.lembar = 'pinTransaksi'; X.gambar(); }); };
})(ExoApp);
